...
`kuma3-preflight-classification/v1`) leads with a **🌐 Global migrations** table — the
cross-cutting fixes (a non-removable field/policy/mesh setting recurring across
`globalSuiteThreshold` suites, e.g. inline `Mesh.mtls`→MeshIdentity+MeshTrust or the shared
`MeshTimeout`/`MeshTrafficPermission` defaults) that are fixed once centrally. Below it,
