...
func TestGatewayMarking(t *testing.T) {
	const (
		titleK8s       = "Kubernetes gateway relies on the kuma.io/gateway annotation"
		titleUniversal = "Universal Dataplane uses the removed kuma.io/gateway marking"
		titleBuiltin   = "Dataplane is a builtin gateway"
	)
	delegated := map[string]any{"type": "DELEGATED", "tags": map[string]any{"kuma.io/service": "gw"}}
	dp := func(env string, gateway map[string]any, labels map[string]any) map[string]any {
		l := map[string]any{"kuma.io/env": env, "kuma.io/workload": "gw"}
		maps.Copy(l, labels)
		it := map[string]any{"labels": l}
		if gateway != nil {
			it["networking"] = map[string]any{"gateway": gateway}
		}
		return it
	}
	for _, tc := range []struct {
		...
		{"k8s annotated gateway", dp("kubernetes", delegated, map[string]any{"kuma.io/gateway": "true"}), titleK8s},
		{"k8s stray pod label", dp("kubernetes", nil, map[string]any{"kuma.io/gateway": "enabled"}), ""},
		{"k8s builtin gateway", dp("kubernetes", map[string]any{"type": "BUILTIN"}, nil), ""},
		{"universal delegated gateway", dp("universal", delegated, nil), titleUniversal},
		{"universal gateway with no type", dp("universal", map[string]any{"tags": map[string]any{"kuma.io/service": "gw"}}, nil), titleUniversal},
		{"universal gateway already labeled", dp("universal", delegated, map[string]any{"kuma.io/gateway": "true"}), titleUniversal},
		{"universal label only", dp("universal", nil, map[string]any{"kuma.io/gateway": "false"}), titleUniversal},
		{"universal builtin gateway", dp("universal", map[string]any{"type": "BUILTIN"}, nil), titleBuiltin},
		{"universal plain proxy", dp("universal", nil, nil), ""},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !slices.Equal(got, want) {
				t.Errorf("gateway findings = %v, want %v", got, want)
			}
		})
	}
}
