...
| B-14 | Confirm the **manual checks** checklist renders | `## Manual checks` section present with the gateway-API / observability / DNS / inspect-API / pod-resources / HMAC-key / mesh-label / MES-zone-routing items. (Unified-naming, inbound-tags, deltaXds, autoReachableServices, global-on-k8s, eBPF and Workload-grouping moved to automated findings — see B-15/B-16.) |
