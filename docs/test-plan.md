...
| B-14 | Confirm the **manual checks** checklist renders | `## Manual checks` section present with the gateway-API / observability / DNS / inspect-API / pod-resources / HMAC-key / MES-zone-routing items. The **mesh-label** item (`kuma.io/mesh` annotation→label, with a `kubectl` validation command) is Kubernetes-only: it appears when the audit observed Kubernetes (a k8s standalone/zone/global, or a k8s zone behind a global) and is absent on a Universal-only run. (Unified-naming, inbound-tags, deltaXds, autoReachableServices, global-on-k8s, eBPF and Workload-grouping moved to automated findings — see B-15/B-16.) |
