...
| **Default outbound access** | transparent proxy without `reachableBackends` reaches every service (`defaults.restrictOutbound: false`, the 2.14 default) | `defaults.restrictOutbound: true` (3.0 default) — no outbounds unless `reachableBackends` lists them | kumahq/kuma#18831, renamed from `allowAllOutbound` in kumahq/kuma#18861/#18862. An explicit `reachableBackends: {}` already meant "none". `/config` cannot tell an explicit `false` from the default, so preflight cannot see a CP that keeps allow-all that way. Preflight flags every transparent proxy with no `reachableBackends` (a Universal Dataplane with a `transparentProxying` section, or any k8s sidecar, since the ConfigMap tproxy path leaves the section nil) and no `backendRef` outbounds (those win over `reachableBackends`), unless its CP (or zone, on a global) already reports `restrictOutbound: true` |
