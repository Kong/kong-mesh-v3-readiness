...
- **Selectors on removed labels** (kumahq/kuma#18392, #17864, #18662): Dataplanes no longer carry `kuma.io/service`, `kuma.io/proxy-type`, `kuma.io/gateway` or other reserved labels outside the registry, so a targetRef/backendRef `labels`, MeshService `dataplaneLabels` or MeshLoadBalancingStrategy `affinityTags` key on one never matches
...
- **`kuma.io/gateway`** is removed (kumahq/kuma#18662): the Pod annotation is ignored, the Dataplane label is an unregistered reserved label (rejected on write) and `networking.gateway` is gone. A delegated gateway becomes an ordinary proxy whose inbound traffic Envoy intercepts; exclude its listen ports with `traffic.kuma.io/exclude-inbound-ports` (plus `kuma.io/ignore: "true"` on its Service) on k8s or `kuma-dp --exclude-inbound-ports` on Universal
