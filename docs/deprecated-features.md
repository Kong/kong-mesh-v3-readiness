...
| **reachableBackends name refs** | `reachableBackends.refs[].name`/`namespace` (Dataplane + `kuma.io/reachable-backends` annotation) | `refs[].labels` (`kuma.io/display-name`, `k8s.kuma.io/namespace`) | kumahq/kuma#18832. Refs without labels resolve to nothing (no outbounds, even with allow-all) and the k8s pod converter rejects the annotation. Preflight flags any ref without `labels` (k8s + Universal) |
