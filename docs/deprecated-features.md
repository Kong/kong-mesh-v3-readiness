...
Outside `experimental:`, `defaults.allowAllOutbound` (`KUMA_DEFAULTS_ALLOW_ALL_OUTBOUND`) follows the same shape: `true` on 2.14, `false` in 3.0. Preflight reports the change as **info** while the switch is still `true` (or absent, on a CP predating the backport — every 2.14 patch up to and including 2.14.5), because `GET /config` serves only the effective value: an explicit `true` is indistinguishable from the 2.14 default, so a blocker there could never be cleared by pinning. Once a control plane sets it to `false` the note goes away, but the two outbound-deny blockers do not: the restriction is live, so a proxy with no `reachableBackends` is already reaching nothing and still has to be configured. Only their framing moves to the present tense. See Outbound denied by default above.
