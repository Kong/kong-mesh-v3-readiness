...
Outside `experimental:`, `defaults.restrictOutbound` (`KUMA_DEFAULTS_RESTRICT_OUTBOUND`) follows the same shape: `false` on 2.14, `true` in 3.0. Preflight reports the change as **info** while the switch is still `false` (or absent, on a CP predating the backport — every 2.14 patch up to and including 2.14.5), because `GET /config` serves only the effective value: an explicit `false` is indistinguishable from the 2.14 default, so a blocker there could never be cleared by pinning. Once a control plane sets it to `true` the note goes away, but the two outbound-deny blockers do not: the restriction is live, so a proxy with no `reachableBackends` is already reaching nothing and still has to be configured. Only their framing moves to the present tense. See Outbound denied by default above.
