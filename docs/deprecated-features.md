...
Outside `experimental:`, `defaults.allowAllOutbound` (`KUMA_DEFAULTS_ALLOW_ALL_OUTBOUND`) follows the same shape: `true` on 2.14, `false` in 3.0. Preflight flags it as a blocker while it is still `true` (or absent, on a CP predating the backport — which is every 2.14 patch up to and including 2.14.5). See Outbound denied by default above.
