...
Outside `experimental:`, `defaults.restrictOutbound` (`KUMA_DEFAULTS_RESTRICT_OUTBOUND`) follows the same shape: `false` on 2.14, `true` in 3.0. Preflight reports the change as **info** while the switch is still `false` (or absent, on a CP predating the backport — every 2.14 patch up to and including 2.14.5), because `GET /config` serves only the effective value: an explicit `false` is indistinguishable from the 2.14 default, so a blocker there could never be cleared by pinning. Once a control plane sets it to `true` the note goes away and the upgrade changes nothing for outbound: a proxy with no `reachableBackends` drops to info (reaching nothing is correct for a workload that calls nothing in the mesh; for any other, setting the field is recommended for security and performance), while a mesh with no MeshPassthrough stays a blocker, worded in the present tense. See Outbound denied by default above.
