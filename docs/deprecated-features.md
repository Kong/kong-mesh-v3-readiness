...
Outside `experimental:`, `defaults.allowAllOutbound` (`KUMA_DEFAULTS_ALLOW_ALL_OUTBOUND`) follows the same shape: `true` on 2.14, `false` in 3.0. Preflight reports the change as **info** while the switch is still `true` (or absent, on a CP predating the backport — every 2.14 patch up to and including 2.14.5), because `GET /config` serves only the effective value: an explicit `true` is indistinguishable from the 2.14 default, so a blocker there could never be cleared by pinning. Once a control plane sets it to `false` it already runs the 3.0 behavior, and both outbound-deny blockers go silent along with the note. See Outbound denied by default above.
